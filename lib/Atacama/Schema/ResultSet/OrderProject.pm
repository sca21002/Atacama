...
sub get_new_result_as_href {
	...
	my %rel =
	  ( projects =>
		  { resultset => 'Project', columns => [ 'project_id', 'name' ] }, );
	...
	while ( my $order_project = $self->next ) {
		...
		foreach my $projectkey (@projectkeys) {
			if ( $projectkey->premiumkey ) {
				my $projectvalue =
				  $order_project->search_related( 'projectvalues',
					{ projectkey_id => $projectkey->projectkey_id } )->single;
...
sub save {
	...
	foreach my $param (@$params) {
		...
		foreach my $key ( keys %$param ) {
			if ( $key eq 'projectoptions' and $row ) {

				if ( $param->{$key} ne 'undef' ) {
